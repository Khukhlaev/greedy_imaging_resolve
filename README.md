...
Scripts for self-calibration and Stokes I imaging with resolve. 

Main idea is running $N+M$ VIs with different random seeds, where $N$ are started from the corresponding MAP estimation, and $M$ are started from the Gaussian prior. The final image can be picked by the $\chi^2_\text{red}$ value, which is provided with the results.
...
To start, specify desired parameters in [`main_conf.cfg`](main_conf.cfg), and run 
...
Modify [`imaging_conf.cfg`](imaging_conf.cfg) only if you want to change the correlation model for images and gains.
...
If some of these parameters are not set with the command line interface, their values will be taken from the main config file. You can review and change these default values in [`main_conf.cfg`](main_conf.cfg).
...
Example of the possible analysis is provided in [`example_analysis.ipynb`](example_analysis.ipynb)
...
[GPLv3](https://www.gnu.org/licenses/gpl.html) and are distributed
